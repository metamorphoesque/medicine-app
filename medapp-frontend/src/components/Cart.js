...
// If you already have AuthContext, replace this mock user with useAuth()
...
const Cart = () => {
  ...
  const needsPrescription = (item) => item.prescription_required === true;
  ...
  const handleCheckout = () => {
    if (!validateCartForCheckout()) {
      alert('Please upload prescriptions for all required medicines before checkout');
      return;
    }
    navigate('/checkout');
  };
      ...
      <div className="cart-container">
        {cartItems.length === 0 ? (
          <div className="empty-cart">
            ...
            <button className="continue-shopping-btn" onClick={() => navigate('/medicines')}>
              Continue Shopping
            </button>
          </div>
        ) : (
                  ...
                  {needsPrescription(item) && (
                    <div className="prescription-requirement">
                      ...
                      {selectedPrescription[item.medicine_id] ? (
                        <span className="rx-uploaded">✓ Uploaded</span>
                      ) : (
                        <button
...
// Export helper to add items to cart
